...
public class ErrorCorrectBarcodes {
	...
	public ErrorCorrectBarcodes (File input, String save) throws IOException{
		...
		while(iter.hasNext()){
			...
			//reads.add(readName);
			reads.add(record.getReadString());
		...
		writeUpdated(merged, barcodeReads);
		
		write(save+".corrected", merged);
	}
	
	
	private void writeUpdated(Map<String, Collection<String>> merged, Map<String, Collection<String>> barcodeReads) {
		for(String originalBarcode: merged.keySet()){
			//if(merged.get(originalBarcode).size()>1){
				int counter=0;
				for(String newBarcode: merged.get(originalBarcode)){
					counter+=barcodeReads.get(newBarcode).size();
				}
				if(!originalBarcode.contains("NOT_FOUND")){
					System.out.println(originalBarcode+"\t"+counter);
				}
				/*for(String newBarcode: merged.get(originalBarcode)){
					if(!newBarcode.contains("NOT_FOUND")){
						System.out.println(originalBarcode+"\t"+newBarcode+"\t"+barcodeReads.get(newBarcode).size()+"\t"+counter);
					}
				}*/
			//}
		}
	...
	public static void main(String[] args) throws IOException{
		...
		String save=args[1];
		new ErrorCorrectBarcodes(file, save);
	}
