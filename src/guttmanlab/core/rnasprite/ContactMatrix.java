...
public class ContactMatrix {
	...
	public ContactMatrix(BarcodingDataStreaming data, Collection<SingleInterval> hub,  Map<String, IntervalTree<SingleInterval>> regions, String save) throws IOException, InterruptedException{
		...
		while(data.hasNext()) {
			Cluster c=data.next();
			//System.out.println(c.getBarcode()+"\t"+c.getClusterSize()+" "+c.getAllDNAIntervals().size());
			if(c.getClusterSize()<maxSize) {
			for(SingleInterval region1: c.getAllDNAIntervals()) {
				String row=getRow(regions, region1);
				//System.err.println(region1.toUCSC()+" "+row);
				if(row!=null) {
	...
	static String usage="RNA \n args[0]=barcoding data (Files) \n args[1]=speckle regions \n args[2]=save";
