...
public class DownsampleSingleCells {

	public DownsampleSingleCells(Map<String, MatrixWithHeaders> data1, int perms,  String save, String binToUse, int numToSample) throws IOException, InterruptedException{
		
		List<String> bins=getBins(data1);
		List<String> rows=getPermNames(perms);
		MatrixWithHeaders merged=new MatrixWithHeaders(rows, bins);
		//merged.setRow("observed", observed.getRow(binToUse));
		
		for(int i=0; i<perms; i++){
			MatrixWithHeaders random=permute(data1, bins, numToSample);
	...
	private List<String> getPermNames(int perms) {
		List<String> rtrn=new ArrayList<String>();
		
		for(int i=0; i<perms; i++){rtrn.add("perm"+i);}
	...
	private MatrixWithHeaders permute(Map<String, MatrixWithHeaders> data1, List<String> bins, int numToSample) {
		MatrixWithHeaders rtrn=new MatrixWithHeaders(bins, bins);
	...
	private List<String> getBins(Map<String, MatrixWithHeaders> data1) {
		Set<String> rtrn=new TreeSet<String>();
		for(String name: data1.keySet()){
	...
	public static void main(String[] args) throws IOException, InterruptedException{
		if(args.length>4){
			...
			int numToSample=new Integer(args[4]);
			new DownsampleSingleCells(data1, numPerms, save, regionToUse, numToSample);
		}
