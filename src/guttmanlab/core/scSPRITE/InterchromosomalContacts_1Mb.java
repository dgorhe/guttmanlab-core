...
public class InterchromosomalContacts_1Mb {

	int numPerms=100000;
	Map<String, Integer> chrSizes=CoordinateSpace.MM10.getRefSizes();
	int binResolution=1000000;
	
	public InterchromosomalContacts_1Mb(File[] files, Collection<SingleInterval> regionsCombined, String save) throws IOException{
		
		Collection<SingleInterval> regions=get1MbBins(regionsCombined);
		...
		Cluster r=kmers.iterator().next();
		List<Cluster> random=getPerms(r, numPerms);
		...
		double[] randomScores=getScores(scores, random);
		for(Cluster kmer: kmers){
			double observed=scores.get(kmer);
			//System.err.println(kmer.toString(false)+"\t"+observed);
			...
			System.err.println(kmer.toString(false)+"\t"+observed+"\t"+average+"\t"+percentile);
			//writer.write(kmer.toString(false)+"\t"+observed+"\n");
		}
		
		writer.close();
		
		
		writer=new FileWriter(save+".random");
		for(int i=0; i<randomScores.length; i++){
			writer.write(randomScores[i]+"\n");
		}
	...
	private Collection<SingleInterval> get1MbBins(Collection<SingleInterval> regionsCombined) {
	...
	private Map<Cluster, Double> score(File[] files, Collection<Cluster> allClusters) throws IOException{
		...
		for(Cluster c: denom.keySet()){
			if(!rtrn.containsKey(c)){rtrn.put(c, 0.0);}
		}
	...
	public static void main(String[] args) throws IOException{
		...
		new InterchromosomalContacts_1Mb(files, regions, save);
