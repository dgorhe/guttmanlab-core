...
public class RunAll {
	...
	private static List<File> subset(File[] allBams, int index) {
		List<File> rtrn=new ArrayList<File>();
		
		int counter=0;
		for(int i=0; i<allBams.length; i++) {
			if(i!=index && allBams[i].getName().endsWith(".bam")) {
				rtrn.add(allBams[i]);
				counter++;
			}
		}
	...
	private static void run(File sampleBam, List<File> controlBams, String save) throws IOException {
		String name=sampleBam.getName().split("\\.")[0];
		
		String cmd="java -jar -Xmx64000m /groups/guttman/mguttman/scripts/RandomDownsample_CLIP.jar";
		cmd+=" "+sampleBam.getAbsolutePath();
		
		for(int i=0; i<controlBams.size(); i++) {
			cmd+=" "+controlBams.get(i).getAbsolutePath();
		}
		cmd+=" "+save;
		
		cmd="srun --nodes=1 --time=24:00:00 --mem=64GB --job-name="+name+ " --output stdout"+name+" --error sterr"+name+ " "+cmd;
	...
	public static void main(String[] args) throws IOException {
		if(args.length>1) {
			...
			for(int i=0; i<allBams.length; i++) {
				if(allBams[i].getName().endsWith(".bam")) {
					File sampleBam=allBams[i];
					List<File> controlBAMs=subset(allBams, i);
