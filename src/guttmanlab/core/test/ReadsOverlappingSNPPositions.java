...
public class ReadsOverlappingSNPPositions {

	static String ambiguous="amb";
	Map<String, String> files;
	
	
	public ReadsOverlappingSNPPositions(File bamFile, File snpFile, String save) throws IOException{
		...
		SAMFileWriter writer1=new SAMFileWriterFactory().setCreateIndex(true).makeSAMOrBAMWriter(reader.getFileHeader(), false, new File(save+".snps"+".bam"));
		...
		while(reads.hasNext()){
			try{
				...
				if(hasOverlappingSNP(record, snpTree)){
					snpCount++;
					writer1.addAlignment(record);
				}
				totalCount++;
				if(totalCount%1000000 ==0){System.err.println(snpCount+" "+totalCount/1000000);}
			}catch (Exception e){e.printStackTrace();}
		}
		
		double ratio=(double)snpCount/(double)totalCount;
		System.err.println("Final " +snpCount+" "+totalCount+" "+ratio);
	...
	private boolean hasOverlappingSNP(SAMRecord record, Map<String, IntervalTree<String>> snpTree) {
		boolean rtrn=false;
		//System.err.println(record.getReferenceName());
		String chr=makeChr(record.getReferenceName());
		if(chr.equals("chrX")){System.err.println(record.getReadName());}
		if(snpTree.containsKey(chr)){
			boolean overlaps=snpTree.get(chr).hasOverlappers(record.getAlignmentStart(), record.getAlignmentEnd());
			if(overlaps){rtrn=true;}
		}
		return rtrn;
	}

	private String makeChr(String referenceName) {
		if(referenceName.contains("chr")){return referenceName;}
		return "chr"+referenceName;
	}

	public ReadsOverlappingSNPPositions(File bamFile, VCFFileReader snps, String fileName, String sp1, String sp2) throws IOException{
		this.files=new TreeMap<String, String>();
		SAMFileReader reader=new SAMFileReader(bamFile);
		
		SAMRecordIterator reads=reader.iterator();
		
		SAMFileWriter writer1=new SAMFileWriterFactory().setCreateIndex(true).makeSAMOrBAMWriter(reader.getFileHeader(), false, new File(fileName+".snps"+".bam"));
		
		
		//Map<String, SAMRecord> snpReads=new TreeMap<String, SAMRecord>();
		...
		while(reads.hasNext()){
			...
			if(hasOverlappingSNP(record, snps, sp1, sp2)){
				//snpReads.put(record.getReadName(), record);
				snpCount++;
				writer1.addAlignment(record);
			}
			totalCount++;
			if(totalCount%100000 ==0){System.err.println(snpCount+" "+totalCount);}
			}catch (Exception e){e.printStackTrace();}
		}
		...
		//write(fileName, reader.getFileHeader(), snpReads);
	...
	private void write(String fileName, SAMFileHeader fileHeader, Map<String, SAMRecord> snpReads) {
		SAMFileWriter writer1=new SAMFileWriterFactory().setCreateIndex(true).makeSAMOrBAMWriter(fileHeader, false, new File(fileName+".snps"+".bam"));
		
		for(String readName: snpReads.keySet()){
			SAMRecord read=snpReads.get(readName);
			writer1.addAlignment(read);
		}
	...
	private void add(Map<String, Pair<SAMRecord>> set1, SAMRecord record) {
		Pair<SAMRecord> pair=new Pair<SAMRecord>(null, null);
		if(set1.containsKey(record.getReadName())){pair=set1.get(record.getReadName());}
	...
	private void mergeSets(Map<String, Pair<SAMRecord>> set1, Map<String, Pair<SAMRecord>> set2,Map<String, Pair<SAMRecord>> set3, String fileName, SAMFileHeader fileHeader, String sp1, String sp2) {
		this.files.put(sp1, fileName+"."+sp1+".bam");
		this.files.put(sp2,fileName+"."+sp2+".bam");
		this.files.put("amb", fileName+".amb.bam");
		
		
		
		SAMFileWriter writer1=new SAMFileWriterFactory().setCreateIndex(true).makeSAMOrBAMWriter(fileHeader, false, new File(fileName+"."+sp1+".bam"));
		SAMFileWriter writer2=new SAMFileWriterFactory().setCreateIndex(true).makeSAMOrBAMWriter(fileHeader, false, new File(fileName+"."+sp2+".bam"));
	...
	private boolean hasOverlappingSNP(SAMRecord record, VCFFileReader snps, String sp1, String sp2) {
	...
	public ReadsOverlappingSNPPositions(File bamFile, Map<SingleInterval, Pair<String>> snps, String fileName) throws IOException{
		SAMFileReader reader=new SAMFileReader(bamFile);
	...
	public static void main(String[] args) throws IOException{
		if(args.length>2){
			...
			File snpFile=new File(args[1]);
			String fileName=args[2];
			/*String species1=args[3];
			String species2=args[4];*/
			new ReadsOverlappingSNPPositions(file, snpFile, fileName);
			//vcf.close();
		}
