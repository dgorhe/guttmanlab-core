...
public class PlotSlidingWindow_FullInsert {

	private int totalSampleCounts;
	private int totalInputCounts;
	private int binSize;

	public PlotSlidingWindow_FullInsert(File sampleBam, File inputBam, String save, int binSize) throws IOException{
	...
	public PlotSlidingWindow_FullInsert(File sampleBam, String save) throws IOException{
		Map<SingleInterval, Double> sampleScores=score(sampleBam, true);
	...
	private TreeMap<SingleInterval, Double> score(File bam, boolean sample){
		...
		while(reads.hasNext()){
			...
			if(!read.getReadUnmappedFlag() && read.getMappingQuality()>1 && read.getFirstOfPairFlag() && !read.getMateUnmappedFlag() && read.getReferenceName().equals(read.getMateReferenceName())){
				Collection<SingleInterval> bin=bin(read, binSize);
	...
	private Collection<SingleInterval> bin(SAMRecord read, int binSize) {
		...
		Collection<SingleInterval> rtrn=binnedInterval(chr, readStart, mateStart, binSize);
		return rtrn;
	}
	...
	public static void main(String[] args) throws IOException{
		if(args.length>3){
			...
			new PlotSlidingWindow_FullInsert(sample, control, save, binSize);
		}
