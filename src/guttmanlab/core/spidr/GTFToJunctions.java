...
public class GTFToJunctions {
		...
		while ((nextLine = reader.readLine()) != null) {
			if(!nextLine.startsWith("#")) {
				...
				if(tokens[2].equals("exon")) {
					...
					if(transcript!=null && transcriptStatus.equals("KNOWN") && type.equals("protein_coding") ) {
	...
	private String getGeneName(String string) {
		return getTag(string, "gene_name");
	}
	
	private String getTag(String string, String tag) {
	...
	private String getTranscriptID(String string) {
		return getTag(string, "transcript_id");
	}
	
	private String getTranscriptType(String string) {
		return getTag(string, "transcript_type");
	}
	
	
	private String getTranscriptSupportLevel(String string) {
		return getTag(string, "transcript_support_level");
	}
	
	
	private String getTranscriptStatus(String string) {
	...
	/*public Collection<SingleInterval> getOverlappingJunctions(SingleInterval junction1) {
		Collection<SingleInterval> rtrn=new TreeSet<SingleInterval>();
		
		if(this.junctionTree.containsKey(junction1.getReferenceName())) {
			Iterator<Gene> iter=this.junctionTree.get(junction1.getReferenceName()).overlappingValueIterator(junction1.getReferenceStartPosition(), junction1.getReferenceEndPosition());
			while(iter.hasNext()) {
				Gene junction2=iter.next();
				if(junction1.getOrientation().equals(junction2.getOrientation())) {
					SingleInterval j2=junction2.getIntrons().iterator().next().getSingleInterval();
					if(j2.overlaps(junction1) || junction1.overlaps(j2)) {rtrn.add(j2);}
				}
			}
		}
		return rtrn;
